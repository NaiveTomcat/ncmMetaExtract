...
def main():
    ...
    if args.mode == "single":
        ...
        process_batch(args.cover_dir, args.base_dir, args.dest)
    else:
        parser.print_help()


def process_batch(cover_dir_str: str, base_dir: str, dest: str | None = None):
    ...
    for i, cover_file in enumerate(cover_files, 1):
        ...
        try:
            processor.process_song(str(cover_file), base_dir, dest=dest)
